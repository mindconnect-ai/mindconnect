...
final class TestDb {
    ...
    /** A connection to the test database with the MCP tables dropped, so every test starts empty. */
    static Sql freshMcpTables() {
        Sql sql = Sql.of(require());
        sql.execute("DROP TABLE IF EXISTS mc_mcp_server; DROP TABLE IF EXISTS mc_mcp_server_import; "
                + "DROP TABLE IF EXISTS mc_mcp_schema_cache");
        return sql;
    }
}
