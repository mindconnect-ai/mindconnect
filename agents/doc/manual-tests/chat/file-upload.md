...
1. New session with an agent that has tools (e.g. `default-chat`); click the
   **+** next to the input → **Upload files** → **Attach…** → `soup.md`.
   ...
   *1 searchable chunk*. There is no chip above the input — attached files
   live behind the **+**, whose **Upload files** entry badges how many there
   are.
2. Reload the page (F5) and open **+** → **Upload files** again.
...
5. Ask a second question about the file.
   **Expected:** The answer is right again; the new user bubble has NO 📎
   ...
   section.
6. In **+** → **Upload files** click **Remove** next to `soup.md` and
   confirm, then ask the same question again.
