    ...
    private Tool listing(UserId user, Accounts<ConnectedMailbox> accounts) {
        return new OfficeTool(LIST,
                "List messages, newest first — headers and the start of the text, not the whole message. "
                        + "Narrow it with from, subject, text (sender or subject), since, before and unread_only; "
                        + "page with offset. With account \"all\" it lists every mailbox's inbox as one. Each line "
                        + "carries the id, account and folder that " + READ + " and the other tools take.",
                object(props(
                        ...
                        "text", string("Only messages with this in the sender or the subject."),
                ...
                args -> {
                    ...
                    MailQuery query = new MailQuery(flag(args, "unread_only", false), str(args, "text"),
                            str(args, "from"), str(args, "subject"),
                            instant(args, "since", zone), instant(args, "before", zone));
                    boolean several = boxes.size() > 1;
                    ...
                    for (ConnectedMailbox box : boxes) {
                        try (MailStore store = mail.open(user, box.id())) {
                            ...
                            String folder = several ? inbox(folders) : folder(folders, str(args, "folder"));
                            MailPage page = several
                                    ? store.list(folder, 0, offset + limit, query)
                                    : store.list(folder, offset, limit, query);
                            for (MailMessage m : page.messages()) rows.add(new Row(box.id(), folder, m));
                            if (page.counted()) matching += page.total(); else counted = false;
                        } catch (MailStoreException e) {
                            if (!several) throw new Refused(box.id() + " did not answer: " + e.getMessage());
                            silent.add(box.id());
                        }
                    }
                    if (several) {
                    ...
                    StringBuilder out = new StringBuilder();
                    if (rows.isEmpty()) {
                        out.append(query.isEmpty() ? "No messages." : "No message matches.").append('\n');
                    } else {
                        out.append("Messages ").append(offset + 1).append('–').append(offset + rows.size());
                        if (counted) out.append(" of ").append(matching);
                        out.append(several ? ", all inboxes" : "").append(", newest first:\n\n");
                        for (Row row : rows) out.append(line(row, true));
                    }
