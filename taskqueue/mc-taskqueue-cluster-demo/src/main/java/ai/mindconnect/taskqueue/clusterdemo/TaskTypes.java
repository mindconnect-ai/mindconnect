...
public class TaskTypes {
    ...
    public TaskTypes(@Value("${mindconnect.cluster.output-root:data/output}") String outputRoot,
                         @Value("${mindconnect.cluster.max-links-per-page-default:5}") int maxLinksPerPageDefault) {
    ...
    public Optional<TaskType> byId(String id) {
        return types.stream().filter(t -> t.id().equals(id)).findFirst();
    }

    private TaskType crawlSite() {
        return new TaskType("crawl", "Crawl website",
    ...
    private TaskType scrapePage() {
        return new TaskType("page", "Scrape single page",
    ...
    private TaskType countdown() {
        return new TaskType(CountdownWorker.TYPE, "Countdown",
