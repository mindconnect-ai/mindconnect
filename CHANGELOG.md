...
- **agents:** **namespaces** — a server now serves any number of them, and a
  ...
  default namespace only). The helper agents a namespace needs — the title
  generator, the summarizers — are created there on first use, on the
  namespace's `agent-default` (or first) LLM config. A chat's title is now
  generated by a task of its own (`agent.title`), submitted together with the
  first turn and run in parallel with it from the user's message alone: it
  shows in the task monitor, survives a restart, runs in the turn's namespace,
  and the turn's handle resolves once both are done — so the title is there
  when the answer is. The task monitor — the header badge, its
  ...
  signed in is refused). Only the creator of a namespace invites, removes members, renames
  it — and deletes it, with everything in it: the directory under the data
  directory, or every row of it in Postgres and its pgvector tables. Any
  other member can leave. Whoever loses a namespace this way is back in the
  default one. A namespace cannot be called `system`, `ns`, `api`, `admin` or
  `v1`. The header switch is a POST. Fixes from a review of the branch: the
  live task stream no longer shows a namespace its viewer was removed from;
  the working-directory root follows the namespace of the call instead of
  ...
  **No migration is shipped:** a Postgres database from 0.8.0 or 0.8.1 still
  has `mc_user` and `mc_api_token` with the namespace in their primary key,
  and the new code cannot write to those tables — every sign-in record,
  namespace switch and token creation fails. Drop both tables before starting
  this version (`DROP TABLE mc_user, mc_api_token;`); they are recreated
  empty, users reappear on their next sign-in.
- **agents:** **skills can be imported from a registry.** An entry of type
  `skill` points at a `SKILL.md`; *Import* stores it as a managed skill, a
  re-import keeps the local id and the enabled flag, and a package can list
  skills next to its agents. The registry screen groups them under *Skills*,
  and removing a package reports the agents that name a skill it would take
  away. The public mc-registry now carries the first ones: Word and
  PowerPoint files through `code_execute`, Deutsche Bahn and Swiss
  public-transport timetables through `bash`.
- **agents:** a registry index that lists **an entry of a kind this version
  does not know** no longer makes the whole registry unreadable: such entries
  are left out, the browse screen says how many, and everything else imports
  as before. Until now a registry gaining a new entry type broke the screen
  for every installation on an older release.
