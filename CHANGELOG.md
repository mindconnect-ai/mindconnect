...
- **agents:** **namespaces** — a server now serves any number of them, and a
  ...
  generated by a task of its own (`agent.title`, behind the turn that produced
  the first exchange): it shows in the task monitor, survives a restart and
  runs in the turn's namespace. In the Admin UI the header shows the current namespace:
