...
| `additionalParams` | object | Provider-specific options. For Anthropic `thinking` (`adaptive`/`disabled`) and `effort` (`low`, `medium`, `high`, `xhigh`, `max`); for OpenAI also `reasoning_summary` (`auto`, `concise`, `detailed`, `none`); for every OpenAI-compatible provider `reasoning_effort` (`none`, `minimal`, `low`, `medium`, `high`, `xhigh` — which levels apply depends on the model; local servers ignore it, and OpenAI is only sent it with a reasoning model, since it rejects it elsewhere). |
